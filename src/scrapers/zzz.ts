...
const ZzzScraper: GameScraper = {
	slug: 'zzz',
	name: 'Zenless Zone Zero',

	scrape: async () => {
		const URL = 'https://zenless-zone-zero.fandom.com/wiki/Redemption_Code'
		...
		try {
					...
					.each((rowIndex, row) => {
						...
						if (cols.length >= 4) {
							...
							let rawCodeText = codeCell
								.text()
								.replace(/Quick Redeem/gi, '') // Hapus teks Quick Redeem
								.replace(/\[.*?\]/g, '')
								.replace(/\u00A0/g, ' ')
			...
			console.error('Error scraping ZZZ:', error)
		}
		return results
	},
}

export default ZzzScraper
